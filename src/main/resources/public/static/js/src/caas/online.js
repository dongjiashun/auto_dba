...
var timelineTemplate = require('dot!./online.dot');
var formTemplate = require('dot!./onlineForm.dot');
...
$(function() {
    ...
    $timeline.on('click', 'a.stash', function () {
        bootbox.alert({
            size: 'large',
            title: "STASH SQL",
            message: '<pre class="code">' + hljs.highlightAuto($(this).data('sql')).value + '</pre>'
        });
    });
    ...
    $('#timeline-btn').click(function () {
        ...
        if (ds && ~~ds > 0) {
            clean();
            $.get(base_path + '/caas/online/' + ds, function (data) {
                if (data && data.length > 0) {
                    ...
                    $.get(base_path + '/task/struct-change/online-process/' + ds, function (tasks) {
                        if (tasks && tasks.length > 0) {
                            ...
                            $timeline.html('该数据源暂时无法提交变更暂存，因为有如下变更正在处理中：<br/><ul class="list-unstyled">' + tasksString + '</ul>');
                        } else {
                            $timeline.html('该数据源暂无需要提交的变更暂存');
                        }
                    });
                }
            });
    ...
    $(document).on('click', '#online-btn', function () {
        ...
        if (task && ~~task > 0) {
            $.get('/caas/online/' + ds + '/' + task, function (data) {
                if (data) {
                    initChangeForm(data);
                } else {
                    $('#change').html('该数据源暂无需要提交的变更暂存');
                }
            });
        }
    });
            ...
            $.ajax({
                ...
                success: function (result) {
                    if(result.code == 0) {
                        ...
                        if (data.left == 0) {
                            bootbox.alert('K2结构变更提交申请成功', function() {
                                location.href = base_path + '/';
                            });
                        } else if (data.left == 1) {
                            bootbox.alert('K2结构变更提交申请失败:未知的数据源');
                        } else if (data.left == 2) {
                            bootbox.alert('K2结构变更提交申请失败:结构变更SQL语法不正确<br/><pre>' + data.right + '</pre>');
                        } else {
                            bootbox.alert('K2结构变更提交申请失败');
                        }
                    }
                },
                complete: function () {
